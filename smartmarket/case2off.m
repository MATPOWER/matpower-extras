function [q, p] = case2off(gen, gencost, discrete)
%CASE2OFF  Creates quantity & price offers from gen & gencost.
%   [q, p] = case2off(gen, gencost, discrete) creates quantity and price offers
...
if discrete
	...
	if any(gencost(:, MODEL) ~= POLYNOMIAL) | any(gencost(:, N) ~= 3)
		error('Cannot convert from non-quadratic gencost to q and p for market type 1');
	end
	q = gen(:, [PMIN, PMAX]);	%% q is just Pmin and Pmax
	m = 2 * gencost(:, COST);	%% slope of marginal costs
	b = gencost(:, COST+1);		%% intercept of marginal costs
	p = [m m] .* q + [b b];		%% p is marginal cost at q
end
