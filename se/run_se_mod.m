...
[V, success, iterNum, z, z_est, error_sqrsum] = ...
    doSEmod(baseMVA, bus, gen, branch, Ybus, Yf, Yt, V0, ref, pv, pq, measure, idx, sigma);

% update Pg and Qg using estimated values idx.idx_zPD
if length(idx.idx_zPG) > 0
    % ** addition to code **
%     cnt = length(idx.idx_zPD) + length(idx.idx_zQD)+ length(idx.idx_zPF)...
%         + length(idx.idx_zPT);
...
if length(idx.idx_zQG) > 0
    % ** addition to code **
%     cnt = length(idx.idx_zPD) + length(idx.idx_zQD)+ length(idx.idx_zPF)...
%         + length(idx.idx_zPT) + length(idx.idx_zPG) + length(idx.idx_zVa)...
%         + length(idx.idx_zQF) + length(idx.idx_zQT);
